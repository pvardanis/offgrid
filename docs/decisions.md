...
## The context window is set on the model's row
...
`--context-window`. The picker sets it in place, on the model's own row: since a
window is asked of a particular model, the control belongs where that model is.
`e` floats a dropdown over that row's `context` cell, over the number already
reading there.

**The `context` column reads the requested window, not the ceiling.** Each row
opens at what that model was last saved at — its last-saved window — and, for a
model never saved, at the model's ceiling. So a glance down the list reads the
windows a run would ask for rather than the room each model happens to have, and
the cell the dropdown floats over is already showing the value it edits.

**The picker always names a concrete number; it never emits `inherit`.** The
field stays optional in the file — a hand edit may omit the window, and absent
means inherit — but the picker does not produce that row. An `inherit` row leaves
the `context` column with nothing to show, since the served window is not
knowable to print, and makes the common case, a model run at the window it last
ran at, the one the screen cannot name. Every row the picker sets names a
number, defaulting to the last-saved window rather than to a fixed guess.

**A dropdown of round windows, stopping at the model's ceiling.** The dropdown
offers the round token counts a window is usually chosen as — 32768, 65536,
131072 and up — up to that row's model ceiling and no further, plus a `Custom…`
entry that reveals a text box for any other valid number. The ceiling is the
model's own, read from the runtime's catalogue; where the runtime states none,
the presets run to the end of the list. Options above the ceiling are not
offered at all rather than shown greyed: the row is being edited for one fixed
...
clamping would hide that the model cannot hold what was asked. The box edits one
row's model, so its ceiling is fixed while it is open.

**The value updates live and is kept per model on a saved run.** Committing the
dropdown changes the in-memory window as the theme's cycling does, and it rides
