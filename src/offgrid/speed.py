...
def tokens_per_second(fit: Fit, machine: Machine) -> float | None:
    ...
    active = _active_bytes(fit)
    efficiency = DENSE_EFFICIENCY if active == fit.weights_bytes else MIXTURE_EFFICIENCY

    return bandwidth * efficiency / active


def _active_bytes(fit: Fit) -> float:
    """Work out what is read per token, as against what is held.

    :param fit: The model, at one of the widths this machine holds it at.

    :return: The weights read for one token. All of them for a dense model,
        and the active share for a mixture.
    """
    listing = fit.listing
    active = listing.active_parameters
    if active is None or active >= listing.parameters:
        return fit.weights_bytes

    return fit.weights_bytes * active / listing.parameters
