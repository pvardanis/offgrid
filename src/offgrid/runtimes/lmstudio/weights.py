...
def read_weights() -> dict[str, int]:
    ...
    """
    try:
        return parse_weights(_list_downloaded(_socket_host()))
...
def _read_call_result(frame: str | bytes) -> list[dict]:
    ...
    :return: The listing the result carried.

    :raise ValueError: When the frame is not this call's result.
    """
    reply = json.loads(frame)
...
def parse_weights(entries: list[dict]) -> dict[str, int]:
    """Read what each downloaded model weighs, keyed on the id it is joined by.

    An entry with no ``modelKey`` or no ``sizeBytes`` is left out rather than
    guessed at: the join tolerates a model the socket did not weigh, and a
    weight nobody can be joined to is a weight about nothing.
    ...
    return {
        ...
        if entry.get("modelKey") and entry.get("sizeBytes") is not None
    }


def attach_weights(models: list[Model], weights: dict[str, int]) -> list[Model]:
