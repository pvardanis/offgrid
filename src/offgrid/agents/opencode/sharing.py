...
saying `disabled` says nothing about a run asking to share on the way past.
"""
...
# it beats `"share": "disabled"` was deliberately not measured, since measuring
# it publishes a real session — offgrid need not know, because it cannot promise
# a transcript stays here while an argument asks for one.
...
def read_transcript_sharing(settings_path: Path, passthrough: Passthrough) -> Reading:
    """Say whether this run could let a transcript leave this machine.

    :param settings_path: The configuration file offgrid writes for this agent.
    ...
    """
    asked = _read_share_argument(passthrough)
    ...
    return _read_share_setting(settings_path)


def _read_share_argument(passthrough: Passthrough) -> str | None:
    ...
    count as typing it; `--no-share` matches neither spelling.

    :param passthrough: What was handed to the agent unchanged.

    :return: The argument, or nothing where none asks it.
    """
    joined = f"{SHARING_ARGUMENT}="
    asking = (a for a in passthrough if a == SHARING_ARGUMENT or a.startswith(joined))

    return next(asking, None)


def _read_share_setting(settings_path: Path) -> Reading:
    ...
    the key as `null`, each say so in their own words: a remedy has to match
    ...
    edit = (
        f"Set {_SETTLED} in {settings_path}, or delete the file and offgrid writes one."
    )
    ...
    if body is None:
        return Reading(
            ...
            detail=(
                f"{settings_path} holds nothing, so nothing says whether a transcript "
                "leaves this machine."
            ),
            remedy="`offgrid run` writes it before it starts the agent.",
        )

    kept = read_as_json(body, settings_path)
    ...
    if not isinstance(kept, dict):
        return Reading(
            ...
            status=Status.UNWRITTEN,
            detail=(
                f"{settings_path} holds {type(kept).__name__} rather than an "
                f"object, so there is no `{SHARING_KEY}` in it to read, and "
                "OpenCode loads no configuration out of it either."
            ),
    ...
    if stated == SHARING:
        return Reading(
            ...
            detail=f"{settings_path} sets `{SHARING_KEY}` to `{SHARING}`.",
        )
    ...
    if SHARING_KEY not in kept:
        return Reading(
            ...
            status=Status.UNWRITTEN,
            detail=(
                f"{settings_path} states no `{SHARING_KEY}`, and measured against "
    ...
    return Reading(
        ...
        status=Status.PERMITTED,
        detail=(
            f"{settings_path} sets `{SHARING_KEY}` to {json.dumps(stated)}, which is "
