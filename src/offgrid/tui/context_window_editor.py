"""The box that floats over a model row to set its context window.

`e` opens it over the highlighted row's `context` cell. A track runs from the
agent's floor to the model's ceiling with a handle on the window the cell shows,
and a box beside it holds the same window as a number a person can type over —
an in-range count moves the handle to it, and a value the model cannot hold is
refused in the words a load would fail with rather than reaching the row.
`escape` abandons, `enter` commits and closes.

Where the runtime states no ceiling there is no right edge to run a track to, so
only the box shows, refused against the floor alone.
