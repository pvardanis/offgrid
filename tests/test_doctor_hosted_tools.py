"""What `offgrid doctor` says about the tools offgrid cannot run for an agent.

A hosted tool runs on its vendor's servers, so against a model held here there
is nothing to run it. `doctor` reports what `run` refuses, and this is where
what it reports is read back.
"""
