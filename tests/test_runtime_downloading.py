...
from offgrid.runtimes import DOWNLOAD_INSTRUCTIONS, describe_download
...
def test_every_runtime_offgrid_names_says_how_a_model_is_downloaded(
    ...
    assert runtime_name in DOWNLOAD_INSTRUCTIONS
...
def test_how_a_model_is_downloaded_names_the_model(runtime_name: RuntimeName):
    ...
    assert MODEL in describe_download(runtime_name, MODEL)
...
def test_how_a_model_is_downloaded_is_said_in_lines_that_fit_a_terminal(
    ...
    # wrote too long.
    said = describe_download(runtime_name, MODEL)
